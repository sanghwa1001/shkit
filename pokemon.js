...
// 원본 8000x249px, 30프레임(가로), 프레임당 정확히 8000/30=266.666...px — 정수로 반올림하면
// 프레임 위치가 누적으로 어긋나므로(프레임 드리프트), JS에서 항상 소수점 그대로 계산해서 사용함
const SHINY_EFFECT_SRC        = 'images/pokemon/layout/shiny.png';
const SHINY_FRAME_COUNT       = 30;
const SHINY_NATIVE_WIDTH      = 8000;
const SHINY_NATIVE_HEIGHT     = 249;
...
function playShinyEffect() {
    ...
    // 프레임이 정사각형이 아니므로(8000x249, 30프레임) 가로 기준으로 배율을 잡고 세로는 비율대로 따라감
    const spriteEl = document.getElementById('monster-sprite');
    const size = spriteEl.clientWidth || spriteEl.offsetWidth || SHINY_NATIVE_WIDTH / SHINY_FRAME_COUNT;
    const scale = size / (SHINY_NATIVE_WIDTH / SHINY_FRAME_COUNT);
    const frameW = (SHINY_NATIVE_WIDTH / SHINY_FRAME_COUNT) * scale; // 반올림 없이 그대로(드리프트 방지)
    const frameH = SHINY_NATIVE_HEIGHT * scale;

    shinyEffect.style.width  = `${frameW}px`;
    shinyEffect.style.height = `${frameH}px`;
    shinyEffect.style.backgroundImage = `url("${SHINY_EFFECT_SRC}")`;
    shinyEffect.style.backgroundSize  = `${frameW * SHINY_FRAME_COUNT}px ${frameH}px`;

    let frameIndex = 0;
    const drawFrame = () => {
        shinyEffect.style.backgroundPosition = `-${frameIndex * frameW}px 0px`;
    };
    drawFrame();
