...
const DEFAULT_AVATARS = ['images/avatars/image_0.gif', 'images/avatars/image_1.gif'];
const ALL_SHOP_AVATARS = ['images/avatars/1.gif', 'images/avatars/2.gif', 'images/avatars/3.gif', 'images/avatars/4.gif', 'images/avatars/5.gif', 'images/avatars/6.gif', 'images/avatars/7.gif', 'images/avatars/8.gif', 'images/avatars/9.gif', 'images/avatars/10.gif'];
...
let runBgX = 0; 
const originalCharacterSrc = "images/game/swim/swimcharacter1.gif"; 
...
// ✅ XSS 방지: HTML 특수문자를 이스케이프해 악성 스크립트 실행을 차단
...
db.ref('chatLog').on('child_added', (snapshot) => {
    ...
    if (data.isAlert) {
        ...
        logDiv.innerHTML += `<div style="margin-bottom: 8px; color: ${color}; font-weight: bold; text-align: center; background: #f8f9fa; padding: 6px; border-radius: 8px; border: 2px dashed ${color}80; font-size:14px; word-break:keep-all;">📢 ${escapeHtml(data.message)}</div>`;
    } else {
        ...
        logDiv.innerHTML += `<div style="margin-bottom: 5px;"><strong style="color:${nameColor}">${escapeHtml(displayName)}:</strong> <span style="color:${msgColor}">${escapeHtml(data.message)}</span></div>`;
    }
...
function renderAdminShopPage() {
    ...
    ALL_SHOP_AVATARS.forEach((avatarId, index) => {
        ...
        const dbKey = avatarId.split('/').pop().replace('.', '_'); 
...
function renderOnlineUsers() {
    ...
    ids.forEach(id => {
        ...
        let bubbleHTML = (user.bubble && user.bubbleTime && (now - user.bubbleTime < 5000)) ? `<div class="speech-bubble">${escapeHtml(user.bubble.length > 15 ? user.bubble.substring(0, 15) + '...' : user.bubble)}</div>` : '';
        let displayId = user.nickname || id; const isMe = (id === currentUser) ? '<span style="color: #ff9800; margin-left:2px;">(나)</span>' : '';
        userDiv.innerHTML = `${requestBadge}${bubbleHTML}<img src="${escapeHtml(user.avatarId || 'images/avatars/image_0.gif')}" alt="아바타" class="online-user-avatar"><div style="display:flex; flex-direction:column; width:100%; flex: 1; justify-content: flex-end;"><span class="name-text-fit ${getNameClass(displayId)}">${escapeHtml(displayId)}</span><span style="font-size:10px; color:#ff9800; font-weight:bold; min-height:12px; line-height:1; margin-bottom:2px;">${(isMe) ? '(나)' : ''}</span></div>`;
...
function renderStudentShop() {
    ...
    localShopItems.forEach(avatarId => {
        const itemDiv = document.createElement('div'); itemDiv.className = 'avatar-item'; const dbKey = avatarId.split('/').pop().replace('.', '_');
        const avatarName = localShopNames[dbKey] || `${avatarId.split('/').pop().split('.')[0]}번 아바타`; const nameText = `<span class="name-text-fit ${getNameClass(avatarName)}">${escapeHtml(avatarName)}</span>`;
...
function buyAvatar(avatarId) {
    const dbKey = avatarId.split('/').pop().replace('.', '_'); const price = localShopPrices[dbKey] !== undefined ? localShopPrices[dbKey] : 1;
...
function renderAvatarList() {
    ...
    (localStudentAccounts[currentUser]?.ownedAvatars || DEFAULT_AVATARS).forEach(avatarId => {
        ...
        let avatarName = (avatarId === 'images/avatars/image_0.gif') ? '기본 남자' : (avatarId === 'images/avatars/image_1.gif') ? '기본 여자' : (localShopNames[avatarId.split('/').pop().replace('.', '_')] || `${avatarId.split('/').pop().split('.')[0]}번 아바타`);
        avatarDiv.innerHTML = `<img src="${escapeHtml(avatarId)}" alt="아바타"><p class="name-text-fit ${getNameClass(avatarName)}" style="margin-bottom:0;">${escapeHtml(avatarName)}</p>`;
...
function renderHighFiveRoom() {
    ...
    ids.forEach(id => {
        ...
        let avatarSrc = (hfState.isStarted && !userIsPaired && !isFailedUser) ? 'images/avatars/unknown.gif' : (realAcc.avatarId || 'images/avatars/image_0.gif'); let displayName = (hfState.isStarted && !userIsPaired && !isFailedUser) ? '???' : (realAcc.nickname || id);
        ...
        userDiv.innerHTML = `${badgeHtml}<img src="${escapeHtml(avatarSrc)}" alt="아바타" class="online-user-avatar"><div style="display:flex; flex-direction:column; width:100%; flex: 1; justify-content: flex-end;"><span class="name-text-fit ${getNameClass(displayName)}">${escapeHtml(displayName)}</span><span style="font-size:10px; color:#ff9800; font-weight:bold; min-height:12px; line-height:1; margin-bottom:2px;">${(isMe) ? '(나)' : ''}</span></div>${statusTextHtml}`; listDiv.appendChild(userDiv);
    });
}
...
function renderWaveRoom() {
    ...
    ids.forEach(id => {
        ...
        let avatarSrc = (waveState.isStarted && !isTeamComplete && !isOffline) ? 'images/avatars/unknown.gif' : (realAcc.avatarId || 'images/avatars/image_0.gif'); let displayName = (waveState.isStarted && !isTeamComplete && !isOffline) ? '???' : (realAcc.nickname || id);
        ...
        userDiv.innerHTML = `<img src="${escapeHtml(avatarSrc)}" alt="아바타" class="online-user-avatar"><div style="display:flex; flex-direction:column; width:100%; flex: 1; justify-content: flex-end;"><span class="name-text-fit ${getNameClass(displayName)}">${escapeHtml(displayName)}</span><span style="font-size:10px; color:#ff9800; font-weight:bold; min-height:12px; line-height:1; margin-bottom:2px;">${(isMe) ? '(나)' : ''}</span></div>${statusTextHtml}`; listDiv.appendChild(userDiv);
    });
}
...
function openSangtiRunGamePage() {
    ...
    setTimeout(() => {
        ...
        if(charWrapper && charImg) {
            ...
            // ✅ 이미지가 이미 캐시돼 있으면 바로 측정, 아니면 로드 완료 후 측정
            if (charImg.complete && charImg.naturalWidth > 0) {
                measureChar();
            } else {
                charImg.onload = measureChar;
            }
        }
    }, 50);
}
...
function spawnRunMonster(){
    ...
    for (let i = 0; i < 2; i++) {
        ...
        monsterImg.src = "images/game/swim/swimmonster" + monsterNum + ".gif";
        ...
        runMonsters.forEach(m => {
            ...
            if(hit){
                ...
                if(m.bubble.textContent === currentRunWord.kor){
                    ...
                    charImg.src = "images/game/swim/swimcharacter2.gif";
                    m.img.src = "images/game/swim/swimmonster" + (m.monsterNum + 1) + ".gif"; 
                    m.img.classList.add("shake");

                    const coin = document.createElement("img"); coin.src = "images/game/swim/swimcoin.gif"; coin.className = "coin-effect";
                    ...
                    charImg.src = "images/game/swim/swimcharacter3.gif"; charImg.classList.add("red-tint");
                    m.img.src = "images/game/swim/swimmonster" + (m.monsterNum + 1) + ".gif";
